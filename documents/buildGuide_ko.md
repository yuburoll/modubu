...
조금 더 튼튼한 고정을 원한다면 위 이미지의 v자 마크가 붙은 부분에 나사를 끼웁니다.(필수는 아닙니다. 너무 세게 조이면 나사 끝이 케이스 바깥으로 살짝 튀어나오니 주의)

이 위에 다음 이미지처럼 조립한 보드를 얹습니다.

pcb 보드를 얹을 땐 바깥쪽의 스위치부터 먼저 보드에 눌러 핫스왑과 결합시킨 다음 나머지를 결합시키는 걸 추천합니다.
